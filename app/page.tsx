...
function BottomNav({tab,setTab}:{tab:Tab;setTab:(t:Tab)=>void}){const tabs=[['id',FileText,'Документ'],['services',Grid2X2,'Сервіси'],['jobs',BriefcaseBusiness,'Вакансії'],['menu',Menu,'Меню']] as const;return <nav className="video-nav" aria-label="Основна навігація">{tabs.map(([id,Icon,label])=><button key={id} className={tab===id?'active':''} onClick={()=>setTab(id)} aria-current={tab===id?'page':undefined}><span className="nav-icon"><Icon strokeWidth={2.15}/></span><span className="nav-label">{label}</span></button>)}</nav>}
