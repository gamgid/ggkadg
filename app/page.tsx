...
function NavGlyph({id}:{id:Tab}){
  if(id==='id')return <svg className="nav-glyph-document" viewBox="0 0 24 28" aria-hidden="true"><rect className="nav-document-body" x="3.5" y="1.5" width="17" height="25" rx="1.5"/><path className="nav-document-lines" d="M6.5 17.5h11M6.5 21.5h11"/></svg>;
  if(id==='services')return <svg className="nav-glyph-services" viewBox="0 0 26 26" aria-hidden="true"><rect x="1.5" y="1.5" width="9" height="9" rx="1.2"/><rect x="15.5" y="1.5" width="9" height="9" rx="1.2"/><rect x="1.5" y="15.5" width="9" height="9" rx="1.2"/><rect x="15.5" y="15.5" width="9" height="9" rx="1.2"/></svg>;
  if(id==='jobs')return <svg className="nav-glyph-jobs" viewBox="0 0 24 28" aria-hidden="true"><rect className="nav-job-body" x="2.5" y="1.5" width="19" height="25" rx="1.4"/><rect className="nav-job-diamond" x="8.6" y="8.6" width="7" height="7" rx=".7" transform="rotate(-40 12.1 12.1)"/></svg>;
  return <svg className="nav-glyph-menu" viewBox="0 0 26 22" aria-hidden="true"><path className="nav-menu-lines" d="M2 2.5h22M2 11h22M2 19.5h22"/></svg>;
}

function BottomNav({tab,setTab}:{tab:Tab;setTab:(t:Tab)=>void}){const tabs=[['id','Резерв ID'],['services','Сервіси'],['jobs','Вакансії'],['menu','Меню']] as const;return <nav className="video-nav" aria-label="Основна навігація">{tabs.map(([id,label])=>{const active=tab===id;return <button key={id} className={active?'active':''} onClick={()=>setTab(id)} aria-current={active?'page':undefined}><span className="nav-icon"><NavGlyph id={id}/></span><span className="nav-label">{label}</span></button>})}</nav>}
